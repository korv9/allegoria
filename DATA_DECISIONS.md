...
- **Date:** 2026-08-20
- **Status:** Superseded in part by DD041 — the `provision_text_sha256` clause no longer describes the code. The rest of this entry (private operational paths, layered lineage timestamps, Bronze hash recomputation) remains Active.
...
- **Consequences:** The v2 payloads are ~70 MB and stay under gitignored `data/local/pool_v2/`; only the manifest, which carries the resolved IDs, titles, versions and SHA-256 hashes, is committed, so the pool is reproducible from git without the repository carrying the bytes. `scripts/pipeline/ingest_sfs_pool.py` writes per document rather than all-or-nothing so a 500-call run is resumable, pins the resolved ID list on first run so `newest 500` cannot drift between runs, and spaces requests by a configurable delay defaulting to 0.5s. Every response is still validated by `parse_sfs_xml` before that document is written, and a document that fails validation is recorded in `failures.json` and skipped rather than ending the run. `scripts/pipeline/build_silver.py`, `build_tables.py` and `query.py` take `--pool v1|v2`; v1 keeps its hard expected counts and v2 has none, because the pool grows whenever it is re-fetched. v2 is a superset of v1 by construction, but re-fetching a law can return a newer consolidated version, so the two pools may hold different `document_snapshot_id` values for the same `document_id`.

## DD041 — Drop `provision_text_sha256`; document-level hashes carry provenance

- **Date:** 2026-09-11
- **Status:** Active; supersedes the `provision_text_sha256` clause of DD038 only
- **Decision:** Silver no longer derives `provision_text_sha256 = sha2(text, 256)`. Provenance is carried by the document-level `source_sha256` and `document_snapshot_id`, which every provision copies, plus the derived `provision_id`. There is currently no per-provision derived-text hash anywhere in the codebase, local or Spark.
- **Rationale:** The column was removed from `products/allegoria/silver/silver_sfs.py` in commit 4da6517, part of the pivot to local development that moved the research core off Databricks. Recording the removal is more honest than leaving DD038 describing a column no consumer produces. Nothing replaced it: at 1,952 provisions the derived text is regenerated from committed Bronze in seconds, so a stored per-row digest bought little.
- **Consequences:** A provision's identity is its `provision_id` plus the document hash it came from; two Silver builds from the same Bronze snapshot are comparable by rebuilding, not by comparing stored row digests. The need re-emerges in Phase 2, where `PROTOCOL.md` requires the corpus to be frozen and hashed before a run — that is a corpus-level hash recorded in the run manifest, not a Silver column, and it should be added there rather than by reinstating this one.
- **Open defect, not fixed here:** the same commit changed Silver to read the Bronze lineage column as `ingested_at`, while `bronze_sfs.py` persists it as `bronze_ingested_at`. Against a real Bronze table Silver would fail to resolve the column. The Databricks pipeline is out of scope for the current local work, so this is recorded rather than corrected; `tests/test_allegoria_product.py` currently asserts the changed form and therefore pins the defect.

## DD042 — Unparsable pool documents are two categories, one of them a parser bug

- **Date:** 2026-09-11
- **Status:** Active
- **Decision:** Treat a v2 pool document that `parse_sfs_html` rejects as belonging to one of two categories, distinguished by the reason and not by the fact of failure. **Category A, correct behaviour (29 of 30):** the document genuinely has no paragraph structure to parse. It is identified by carrying no `§` in its text at all (28 documents), or by opening with `Har upphävts genom lag (...)` where Riksdagen serves a repealed law without `a.paragraf` anchors (1 document, `sfs-2014-1475`). In the v2 pool these are 13 repealing acts (`Lag om upphävande av ...`), 14 tax-information-exchange acts whose operative text is a single sentence and whose bulk is a treaty annex in `Artikel` form rather than `§` form, and 2 one-sentence acts. **Category B, a parser defect (1 of 30):** `sfs-2023-692` fails with `transitional provisions contain no SFS markers` even though it carries 10 well-formed paragraphs, `1 §` through `10 §`.
- **Rationale:** The two look identical from the outside — a skipped document — but mean opposite things. Category A is the parser correctly declining text that has no provisions. Category B is the parser discarding ten valid provisions because of an unrelated section at the end of the document. Counting them together would have hidden the defect behind an acceptable-looking skip rate.
- **The defect:** `_transitional_records` requires an SFS marker matching `(?m)^(\d{4}:\d+)$`, a bare number alone on a line. `sfs-2023-692` writes it as `SFS 2023:692` with the prefix on the same line as the first transitional item. The marker is present and readable; only its position and prefix differ. A document whose transitional block cannot be split is rejected in full, so the failure of one trailing section destroys every paragraph in the document.
- **Measured: v1 is not affected.** All 50 v1 documents parse, and of the 23 carrying a transitional section, the strict regex finds all 76 markers — exactly the 76 `transitional_provision` rows in Silver. Zero v1 documents use the `SFS NNNN:NN` inline form, so nothing is being silently dropped inside the 1,952. The exposure is latent, not realised: any future law written in that form would lose all of its paragraphs, not just its transitional ones.
- **Consequences:** `Pool.strict = False` for v2 stays for now, but it is masking Category B rather than only Category A, so it is not a permanent answer. Fixing the marker regex means editing `products/allegoria/sfs_parser.py`, which is shared with the Databricks Silver notebook and therefore governs the frozen v1 counts; it is not changed unattended. Recommended fix, for a human to approve: accept an optional `SFS\s+` prefix and stop requiring the marker to occupy a line alone. Until then, a `transitional provisions contain no SFS markers` failure must be read as a bug report, never as a skip.

## DD043 — Extract `simulacria/`; `products/` keeps its name

- **Date:** 2026-09-11
- **Status:** Active
- **Decision:** The selection heuristics move out of `scripts/` into an installed `simulacria` package, split into `simulacria.selection` (marker sets, the determinacy ladder, pools, scoring, table access) and `simulacria.measurement` (empty until Phase 2, reserved for `direction.py` and `contracts.py`). `scripts/` becomes thin CLI wrappers that export nothing and import only from the package. `products/` keeps its name even though it is no longer the only product tree and is no longer Databricks-only in spirit.
...
## DD048 - Layered packages, and datasets as a parameter

- **Date:** 2026-09-12
- **Status:** Active
- **Decision:** `simulacria` is split into `domains/`, `pipeline/`, `generation/`, `measurement/`, `reporting/` and `selection/`; scripts are grouped into `pipeline/`, `run/`, `report/` and `investigations/`. A dataset is a domain adapter plus a corpus YAML (`schema_version: 2`, now carrying `domain` and its own reading questions) plus an experiment config under `configs/`. `generation` and `measurement` resolve passages through `simulacria.domains.get(name)` and may not import a concrete domain.
- **Rationale:** The user asked for a structure that makes the same test runnable on other datasets, such as insurance terms or English RFCs. Swedish statute was wired into the core in three places: the corpus loader parsed SFS bronze directly, the reading questions were a Python dict keyed by Swedish slot ids, and the plan hardcoded which corpus got which styles. Each of those is now data.
- **Consequences:** `claude-sonnet-5`/`claude-haiku-4-5-20251001` and every protocol rule are unchanged; the instrument is deliberately not configurable, because two corpora are only comparable while the slot vocabulary, blinding, reading schema and receipts stay identical. Three configs ship: `pilot-sv`, `allegoria-sv`, `rfc-en`. The English domain (`rfc`) is a demonstration that the seam holds, not a replication, and its results are never pooled with the Swedish ones. Corpus files moved to `schema_version: 2`; `statutory_axis_v1.yaml` stays an axis input with no domain. Prompts moved to `prompts/sv/` and `prompts/en/`. The one partial OpenAI run is untouched and still refused by name. 66 tests pass and the one frozen determinacy fixture still fails by design. An `eda/` folder holds five model-free exploratory notebooks.

## DD049 - Model choice is configuration, with a provider seam

- **Date:** 2026-09-12
- **Status:** Active
- **Decision:** `configs/models.yaml` declares every usable model (provider, wire id, prices, parameters, structured-output support, verification date); each experiment config names a `transformer` and an `extractor` by registry name, overridable per invocation with `--transformer` / `--extractor`. `simulacria/providers/` holds one module per API behind a fixed contract, and `simulacria/generation/provider.py` dispatches by model id. Every run's manifest records the full resolved spec of both models.
- **Rationale:** The user asked to be able to test different models and different LLM APIs depending on what they have access to. Model ids, prices and sampling parameters were constants in code, so switching a model was an edit to the hot path and a stale price silently made the budget guard wrong. Prices are facts about the world that change without any code changing, so they belong in data.
- **Consequences:** Adding a model is a registry entry; adding an API is a provider module; neither is a change to the experiment. `verified:` records the date someone actually called a model through this code, and `scripts/investigations/check_provider.py` performs that check for a fraction of a cent; the run commands refuse an unverified model unless `--allow-unverified-model` is passed. The OpenAI provider is implemented from the documented Responses API shape and covered only by offline fixtures, so both its entries stay unverified and its prices need re-checking before paid use. Each provider decides which keys it accepts, because an Anthropic key also starts with `sk-` and a bare prefix check would have leaked it to another vendor. The instrument -- slot vocabulary, blinding, reading schema, receipts, budget and retry scopes -- is deliberately not configurable. 81 tests pass; the one frozen determinacy fixture still fails by design.

## DD050 - OpenAI verified; cross-provider model comparison

- **Date:** 2026-09-12
- **Status:** Active
- **Decision:** The OpenAI provider is verified (`gpt-5-4-mini`, `gpt-5-4-nano`, 2026-09-12) and both models are usable from a config. `scripts/investigations/compare_models.py` records a cross-provider comparison of three writers and four readers on the real prompts and sources. The default pair is unchanged pending the user's decision; the evidence recommends `gpt-5-4-mini` as reader.
- **Rationale:** The user's OpenAI account moved from 50 requests per day to 5,000 per minute, which removes the reason this project left OpenAI (DD046). A comparison was needed before spending on a full run, and it had to run through the pipeline's own request path so it tested the provider seam as well as the models.
- **Consequences:** Measured over 66 calls costing $0.30: as reader, `gpt-5-4-mini` was valid on 3/3 sources and 8/9 allegories at $0.0009 per reading, against `haiku-4-5` at 2/3 and 6/9 for $0.0033 and `opus-5` at 3/3 and 9/9 for $0.0184; agreement with `opus-5` was 96% for mini, 97% for haiku, 92% for nano. As writer, all three models followed every instruction; allegory expansion was 2.61x (`sonnet-5`), 2.12x (mini), 1.77x (nano). A full Swedish run costs $18.86 with Opus reading, $5.66 with Haiku, $3.60 with mini, and $0.88 for an all-OpenAI pairing. Twelve readings per reader rank validity but do not estimate a rate, and 95-98% of all slot answers were `present`, so no reader has yet demonstrated shift detection on this material. `requests_per_minute` is recorded in the registry where the dashboard states it, and the run command suggests a pacing interval from it rather than guessing. OpenAI prices remain unconfirmed. Switching the reader will require an amendment to `predictions/2026-09-11-amendment-cheapest-models.md` before a run is created.
