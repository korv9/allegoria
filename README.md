...
simulacria/                     PLANNED — research core, none of this exists yet
|-- contracts.py                Pydantic models
|-- direction.py                deterministic classification
`-- loop.py                     recursive transformation
...
scripts/                        local entrypoints, no Spark anywhere
...
`-- ...                         gold-set sampling, scoring and renderers

review/                         the reviewable output of each working session
...
[`review/SPECIMENS.md`](review/SPECIMENS.md), twinned and fully annotated by hand — duty,
...
- **The wildcard-gap marker audit is open.** `om inte` and `får inte` both use a `\w+` gap standing
  ...
  v2 hits. A fix is scored and proposed in [`review/REVIEW.md`](review/REVIEW.md) but not shipped:
...
- **`ceiling` is pending ratification.** `dock` marks both a carve-out from a duty and an upper
  ...
  [`review/DIRECTION-ceiling-proposal.md`](review/DIRECTION-ceiling-proposal.md);
