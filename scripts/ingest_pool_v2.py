...
`data/source/sfs/manifest_v2.json` so the pool stays reproducible from git.
    ...
    python scripts/ingest_pool_v2.py                 # 500 laws, 0.5s apart
    python scripts/ingest_pool_v2.py --count 100
    python scripts/ingest_pool_v2.py                 # re-run: skips what exists
...
def write_manifest(pool_dir: Path, document_ids: list[str], manifest_path: Path) -> int:
    ...
    manifest = {
        ...
        "payload_location": (
            "data/local/pool_v2/ -- gitignored; rebuild with scripts/ingest_pool_v2.py"
        ),
        "supersedes": None,
        "relation_to_v1": (
