...
v1 snapshot it must produce the same rows: 1,952 provisions from 50 documents.
    ...
    python scripts/pipeline/build_silver.py              # v1, the frozen 50-law corpus
    python scripts/pipeline/build_silver.py --pool v2    # the larger selection pool

A thin wrapper. The build lives in `simulacria.pipeline.silver`.
"""
...
from simulacria.pipeline.silver import POOLS, build
