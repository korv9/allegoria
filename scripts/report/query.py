...
it, and prints the result. No ORM, no query builder, no schema layer.
    ...
    python scripts/report/query.py -c "select determinacy, count(*) from candidates group by 1"
    python scripts/report/query.py < docs/some_query.sql
    echo "describe provisions" | python scripts/report/query.py
