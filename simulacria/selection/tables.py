"""SELECTION. Read access to the Parquet analysis tables.

Not measurement. These tables hold marker hits and candidate rankings -- the
output of the heuristics -- and exist so a human can interrogate the shortlist
with SQL instead of reading a terminal dump.

`scripts/build_tables.py` writes them; everything else reads them through here.
"""
