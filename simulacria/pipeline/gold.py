"""GOLD. Rebuildable SQL projection of verified evidence, never the source of record.

Selection tables are snapshotted from Parquet; research tables are built from run
directories that `reporting.runs.load_run` has replay-verified first. Everything
here can be deleted and rebuilt; the runs it reads cannot.
"""
...
from simulacria.reporting.runs import RetiredProviderRun, load_run
    ...
    for name in TABLE_NAMES:
        if not (tables / f"{name}.parquet").is_file():
            raise FileNotFoundError(
                f"Missing {name}; run scripts/pipeline/build_tables.py --pool {pool}"
            )
