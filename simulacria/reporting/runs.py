...
def latest_run(root: Path) -> Path:
    ...
    if not manifests:
        raise FileNotFoundError(
            "No actual generation-one run exists. Add your provider key to .env and run "
            "python scripts/run/pilot.py. No model outputs have been simulated."
        )
...
def latest_recursive_run(root: Path) -> Path:
    ...
    if not readable:
        raise FileNotFoundError(
            ...
            "python scripts/run/chains.py. No model outputs have been simulated."
        )
